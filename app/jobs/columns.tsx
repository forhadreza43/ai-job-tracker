...
export const columns: ColumnDef<Job>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const date = row.getValue('interviewDate') as Date | null;
      return date ? (
        new Date(date).toLocaleDateString()
      ) : (
...

  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Link
          className="bg-secondary px-2 py-1 rounded-md hover:bg-primary hover:text-white transition-all duration-300"
          href={`/jobs/${job.id}`}
        >
          Details
        </Link>
